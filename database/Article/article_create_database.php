...
if(!empty($_POST['title']) && !empty($_POST['smallDesc']) && !empty($_POST['content']) && !empty($_POST['categorie'])){
    $create_article = $bdd->prepare('INSERT INTO article(id_user,title,smallDesc,content,categorie,createdAT) VALUES(:id_user,:title,:smallDesc,:content,:categorie,NOW())');
